...
const rewardsContract_stake = async function(stakeTokenAddr, rewardPoolAddr, App, maxAllowance) {
  ...
  if (currentTEND / 1e18 > 0) {
    ...
    allow
      .then(async function() {
        WEEBTEND_V2_TOKEN.stake(currentTEND, {gasLimit: 200000})
          .then(function(t) {
...
const rewardsContract_unstake = async function(rewardPoolAddr, App) {
  ...
  if (currentStakedAmount > 0) {
    showLoading()
    REWARD_POOL.withdraw(currentStakedAmount, {gasLimit: 200000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
...
const rewardsContract_claim = async function(rewardPoolAddr, App) {
  ...
  if (earnedYFFI > 0) {
    showLoading()
    REWARD_POOL.getReward({gasLimit: 200000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
...
const boardroom_claim = async function(rewardPoolAddr, App) {
  ...
  if (earnedYFFI > 0) {
    showLoading()
    REWARD_POOL.claimReward({gasLimit: 200000})
      .then(function(t) {
        return App.provider.waitForTransaction(t.hash)
      })
      .catch(function() {
...
const chefContract_claim = async function(chefAbi, chefAddress, poolIndex, App, 
  ...
  if (earnedTokenAmount > 0) {
    showLoading()
    if (claimFunction) {
      claimFunction(poolIndex, {gasLimit: 200000})
        .then(function(t) {
          return App.provider.waitForTransaction(t.hash)
        })
    }
    else {
      CHEF_CONTRACT.deposit(poolIndex, 0, {gasLimit: 200000})
        .then(function(t) {
          return App.provider.waitForTransaction(t.hash)
        })
        .catch(function() {
...
function printAPR(rewardTokenTicker, rewardPrice, poolRewardsPerWeek, 
  ...
  _print(`${rewardTokenTicker} Per Week: ${poolRewardsPerWeek.toFixed(0)} ($${formatMoney(usdPerWeek).toFixed(0)})`);
  ...
  _print(`You are staking ${userStaked.toFixed(fixedDecimals)} ${stakeTokenTicker} ($${formatMoney(userStakedUsd).toFixed(0)}), ${userStakedPct.toFixed(2)}% of the pool.`);
